...
import pea.app.compiler.ReloadableClassLoader.getExtJarsUrlClassLoader
...
private[app] object SimulationClassLoader {
  ...
  //  var reloadCl: ReloadableClassLoader = null

  def apply(binariesDirectory: String = PeaConfig.defaultSimulationOutputFolder): SimulationClassLoader = {
    //    if (reloadCl == null)
    //      reloadCl = new ReloadableClassLoader(Thread.currentThread().getContextClassLoader, binariesDirectory)
    println("create SimulationClassLoader isUnloaded:"+ReloadableClassLoader.isUnloaded)
    val reloadableClassLoader = new ReloadableClassLoader(baseDir = binariesDirectory)
    if (ReloadableClassLoader.isUnloaded) {
      ReloadableClassLoader.resetJarLoader()
    }
    new SimulationClassLoader(reloadableClassLoader, binariesDirectory)
  }
