...
class HomeApi @Inject()(
  ...
  def report(path: String) = Action {
    ...
    if (file.isDirectory) {
      ...
      if (file.exists()) {
//    todo 直接看报告    if (file.getCanonicalPath.startsWith(PeaConfig.resultsFolder)) {
...
// todo 跟 /api/gatling/compile重复了   if (PeaConfig.enableReporter) PeaConfig.workerActor ! AsyncCompileMessage(pull = message.pull)
    PeaService.compileWorkers(message.workers, message.pull).toOkResult
  }

  def arthasUrl() = Action(parse.byteString) { implicit req =>
    OkApiRes(ApiRes(data = PeaConfig.defaultArthasUrl))
  }
