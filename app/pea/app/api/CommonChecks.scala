...
trait CommonChecks extends CommonFunctions {
  ...
  def checkTestFolder(func: => Result): Result = {
    if (StringUtils.isNotEmpty(PeaConfig.defaultSimulationSourceFolder)) {
      func
    } else {
      ErrorResult("Config 'scripts folder test' is not set")
    }
  }
