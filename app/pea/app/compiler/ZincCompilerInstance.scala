...
object ZincCompilerInstance {
  ...
  def build(classpath: String): ZincCompilerInstance = {
    try {
      var classpathFiles: Array[File] = classpath.split(File.pathSeparator).map(new File(_))
      ...
      val scalaInstance = {
        ...
        //从父类加载器里获取classpath，并添加
        var urls = getClass.getClassLoader.asInstanceOf[URLClassLoader].getURLs
        urls.foreach(f => {
          classpathFiles = classpathFiles :+ new File(f.getFile)
        })
        if (getClass.getClassLoader.getParent != null && getClass.getClassLoader.getParent.getParent != null) {
          urls = getClass.getClassLoader.getParent.getParent.asInstanceOf[URLClassLoader].getURLs
          urls.foreach(f => {
            classpathFiles = classpathFiles :+ new File(f.getFile)
          })
        }
        ...
        new ScalaInstance(
          ...
          //   默认父类加载器事AppClassLoader，会去app加载器里拿，因为是-cp启动的，sbt-lanuch.jar里的scala库是不全的
//           loader = new URLClassLoader(allScalaJars.map(_.toURI.toURL)),
          loader = new URLClassLoader(allScalaJars.map(_.toURI.toURL), getClass.getClassLoader),
